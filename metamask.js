...
function fund(){
    ...
    try {
        ...
        let usdtTx={
            from:account,
            to:usdtAddress,
            data:UsdtInstance.methods.approve(contractAddress,amount)
        }
        let approve = web3.eth.sendTransaction(usdtTx).encodeABI()
        ...
        let buy = web3.eth.sendTransaction(presaleTx).encodeABI()
        buy.then((payload) => {
            alert(payload)
        }
        ).catch((e) => {
